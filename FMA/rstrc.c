  ...
  for (jf=3,jc=2;jc<nc;jc++,jf+=2) { 
    for (iif=3,ic=2;ic<nc;ic++,iif+=2) {
      	...
      	uc[ic][jc][kc]=0.5*uf[iif][jf][kf] + 0.0833*
				...
				+ uf[iff][jf][kf+1] + uf[iff][jf][kf-1]
				);
    }
  }
  /* Boundary points. */
	// for changing k's
  for (jc=1,ic=1, kc=1;ic<=nc;ic++,jc+=2,kc+=2) { 
  ...
  for (jc=1,ic=1, kc=1;ic<=nc;ic++,jc+=2,kc+=2) {
	...
	for( kc=1, kcc=1; kc <= nc; kc++, kcc+= 2 )
		...
		{	
			...
			uc[ic][nc][kc]=uf[jc][nc][kcc];
		}

	for( kc=1, kcc=1; kc <= ncc; kc++, kcc+= 2 )
		for( jc=1, ic=1; ic<=nc; ic++, jc+=2 )
		{	
			uc[1][ic][kc] =uf[1][jc][kcc];
			uc[nc][ic][kc]=uf[nc][jc][kcc];
		}
		
	for( jc=1, ic=1; ic<=nc; ic++, jc+=2 )
	{	
		uc[ic][ic][1] = uf[jc][jc][1];
		uc[ic][ic][nc] = uf[jc][jc][nc];
	}
}
